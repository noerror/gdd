---
title: 밸런스 검증 & 시뮬레이션 작성 가이드
description: 게임 밸런스를 체계적으로 검증하고 시뮬레이션하여 출시 전 문제를 사전에 발견하고 데이터 기반으로 최적화하는 검증 메소드 가이드
---

# 밸런스 검증 & 시뮬레이션 작성 가이드

## 1. 목적 (Purpose)
게임 밸런스를 체계적으로 검증하고 시뮬레이션하여 출시 전 문제를 사전에 발견하고 해결하기 위한 가이드입니다.

## 2. 범위 (Scope)
- **포함**: 밸런스 검증 방법론, 시뮬레이션 설계, 플레이테스트 계획, 데이터 분석
- **제외**: 구체적인 시뮬레이션 코드, 통계 분석 도구 구현, UI/UX 구현

## 3. 설계 목표 (Design Goals)

1. **사전 문제 발견 (Early Detection)**: 출시 전 밸런스 문제를 사전에 발견하고 해결합니다.
2. **데이터 기반 의사결정 (Data-driven)**: 주관적 판단이 아닌 데이터 기반으로 밸런스를 조정합니다.
3. **반복적 개선 (Iterative Improvement)**: 지속적인 테스트와 조정으로 밸런스를 최적화합니다.
4. **측정 가능성 (Measurability)**: 밸런스 품질을 정량적으로 평가할 수 있는 지표를 제공합니다.

## 4. GDD 문서 구조 (Structure)

### 4.1 검증 방법론 (Validation Methodology)
- **검증 목표**: 밸런스 검증의 핵심 목표
- **검증 방법**: 시뮬레이션, 플레이테스트, 데이터 분석

### 4.2 시뮬레이션 (Simulation)
- **시뮬레이션 설계**: 모델링, 파라미터, 시나리오
- **실행 계획**: 반복 횟수, 데이터 수집

### 4.3 플레이테스트 & 분석 (Playtest & Analysis)
- **플레이테스트 계획**: 대상, 시나리오, 피드백 수집
- **데이터 분석**: 지표, 통계 분석, 조정 방안

### 4.4 선택 및 심화 (Optional & Advanced)
- **자동화 시뮬레이션**: 헤드리스(Headless) 클라이언트를 이용한 1000배속 고속 시뮬레이션 환경
- **데이터 저장**: 대용량 로그 처리를 위한 시계열 DB(InfluxDB 등) 활용 및 데이터 수명 관리
- **시각화**: 밸런스 지표 실시간 대시보드(Grafana) 구축 및 알림(Alert) 설정

## 5. 관련 개념 (Key Concepts)

- **Monte Carlo Simulation**: 무작위 샘플링을 통한 확률적 시뮬레이션
- **A/B Testing**: 두 가지 버전을 비교하여 최적안을 선택
- **Playtest Metrics**: 플레이테스트에서 수집하는 정량적 지표
- **Statistical Significance**: 통계적 유의성 검증
- **LP 메타게임 바운드**: 매치업 행렬을 선형계획법으로 풀어 캐릭터별 픽률 상·하한을 출시 전에 예측하는 기법 (Alex Jaffe)
- **Markov Chain**: 상태 전이 행렬로 반복 시스템의 정확해를 구하는 도구 (모노폴리 칸별 도달 확률 → 부동산 ROI)
- **Fictitious Play**: 신규 플레이어가 '현 메타의 카운터'를 골라 진입하는 과정의 게임이론 모델. 커뮤니티 구성비가 셀렉션 게임의 최적 혼합 전략으로 수렴하는 근거

## 6. 품질 체크리스트 (Quality Checklist)

### 6.1 검증 설계
- [ ] 검증 목표가 명확하게 정의되었는가?
- [ ] 검증 방법이 목표에 적합한가?
- [ ] 측정 가능한 지표가 정의되었는가?

### 6.2 시뮬레이션
- [ ] 시뮬레이션 모델이 게임 메커닉을 정확히 반영하는가?
- [ ] 충분한 반복 횟수로 통계적 유의성을 확보했는가?
- [ ] 다양한 시나리오가 테스트되었는가?
- [ ] 같은 확률 문제를 수식과 몬테카를로 양쪽으로 풀어 일치를 확인했는가?
- [ ] 스프레드시트 → 게임 데이터 내보내기가 자동화되어 있는가? (수작업 입력은 오타 사고의 시한폭탄)
- [ ] 대전형 게임이라면 매치업 행렬 기반 메타게임 예측(픽률 바운드)을 수행했는가?

### 6.3 플레이테스트
- [ ] 플레이테스트 대상이 타겟 플레이어를 대표하는가?
- [ ] 피드백 수집 방법이 체계적인가?
- [ ] 데이터 분석 결과가 밸런스 조정에 반영되었는가?
- [ ] 테스터의 숙련도 편향(개발자 = 그 게임의 숙련자)에 핸디캡 보정을 했는가?
- [ ] 출시 전 KPI와 실패 시나리오별 조치가 사전 합의되어 있는가?

## 7. 예시 및 안티패턴 (Examples & Anti-Patterns)

**Case 1: 시뮬레이션 검증**
- **좋은 예 (Good)**: "1000회 시뮬레이션으로 드롭 확률 검증, 통계적 유의성 확보, 예상 밖의 결과 사전 발견"
- **안티패턴 (Bad)**: "주관적 판단만으로 밸런스 조정하여 문제 발생"

**Case 2: 플레이테스트**
- **좋은 예 (Good)**: "다양한 스킬 레벨의 플레이어 20명 대상 테스트, 정량적 지표(완료 시간, 사망 횟수) 수집, 피드백 기반 조정"
- **안티패턴 (Bad)**: "소수의 개발자만 테스트하여 플레이어 관점 누락"

**Case 3: 데이터 분석 (Albion Online)**
- **좋은 예 (Good)**: "수학적 모델링으로 경제 밸런스 예측, 플레이어 피드백 통합, 반복적 밸런싱"
- **안티패턴 (Bad)**: "데이터 수집 없이 출시 후 문제 발견"

**Case 4: 몬테카를로 시뮬레이션 (Dead Cells)**
- **좋은 예 (Good)**: "무기 드롭 확률 검증 : 1,000회 시뮬레이션 → 예상 드롭률 18% vs 실제 18.2% (오차율 0.2%), 강화 성공률 : 레벨별 성공률 70~95% 범위 테스트 → 1만회 반복으로 통계적 유의성 확보(신뢰도 95%), 경제 밸런스 : 보상 및 손실 시뮬레이션 → 평균 플레이 시간 45분 당 순익 2,500~3,500 골드 범위 확인", "테스트 결과로 드롭 테이블 조정 필요 구간 사전 식별"
- **안티패턴 (Bad)**: "몇 번의 수동 테스트만 진행 → 출시 후 특정 무기 드롭률이 0%에 가까운 버그 발견"

**Case 5: 플레이테스트 데이터 수집 (Hades)**
- **좋은 예 (Good)**: "플레이테스트 대상 : 난이도별 플레이어 20명 × 3회 = 60회 세션, 수집 지표 : 평균 클리어 시간, 사망 횟수, 사용 무기 조합, 특정 보스별 재시도 횟수, 사용자 성공률 목표 50~60% 달성(충분한 도전감 + 높은 완료율), 데이터 분석 결과 : 특정 조합(검 + 핸드캐논)의 성공률 75% → 마이너 너프 +2% 데미지 감소, 반대로 약한 조합(활 + 권총) 성공률 35% → 버프 적용"
- **안티패턴 (Bad)**: "개발팀 5명만 테스트 → 실제 플레이어 수준의 오류 미발견"

**Case 6: A/B 테스트 & 반복적 개선 (Cuphead)**
- **좋은 예 (Good)**: "버전 A : 보스 체력 100%, 패턴 속도 1.0배 / 버전 B : 체력 110%, 패턴 속도 0.95배, 각 버전을 50명씩 테스트 → 평균 클리어 시간 A: 12분, B: 11분, 재시도 횟수 A: 8.5회, B: 7.2회, 통계 유의성 검증(p<0.05) → B 버전(체력 증가 + 속도 감소) 채택", "패턴별 난이도 : 3단계 공격 패턴 A 난이도 5, 2단계 B 난이도 3 으로 정량화 → 보스 전체 난이도 = 평균 4.2 (목표 4.0~4.5 범위 내)"
- **안티패턴 (Bad)**: "한 가지 버전만 테스트 → 더 나은 대안 존재 여부 미확인"

**Case 7: LP 메타게임 예측 (PlayStation All-Stars)**
- **좋은 예 (Good)**: "매치업 행렬을 선형계획법으로 풀어 캐릭터별 픽률 상·하한(메타게임 바운드)을 출시 전에 산출 / Sackboy는 평균 매치업 전체 3위로 평범해 보였지만 LP는 '~50% 픽 강제'를 예고 → 실서비스 픽률 18% → 25% → 40%+로 폭주(승률은 내내 불변)하며 예측 검증 / 3종 병리 탐지: 항상 배제(최대 빈도 0% — 버프 후보), 때때로 지배, 항상 지배(수술 대상)"
- **교훈**: "픽률 요동은 파워 변화가 아니라 '학습'이다 — 현 메타 스냅샷으로 밸런스를 판정하지 마라. 전문가들이 추정한 매치업 표라도 티어리스트보다 낫다(잘 정의되고, 픽 빈도에 둔감하고, 실데이터로 근사 검증 가능)"

**Case 8: 개발자 핸디캡 테스트 (스매시브라더스)**
- **좋은 예 (Good)**: "개발 스태프는 그 게임의 숙련자이므로 '개발자가 아슬아슬하게 클리어'로 맞추면 일반 유저는 끝까지 못 감 → 한 손 플레이·체력 축소·조건 엄격화 같은 물리적 핸디캡을 걸고 확인 / 얼티밋부터 유력 플레이어 모니터반이 매일 대전 + 온라인·대회 데이터 병용, 디렉터 개입은 '조정이 개성을 죽이려 할 때'로 한정"
- **안티패턴 (Bad)**: "개발팀 기준 튜닝 — 자신들을 표준이라 여기지 말고 '생각보다 더 낮추는' 정도가 적당하다"

## 8. 핵심 인사이트 (Key Insights)

### 기본 원칙
- **수학적 모델링 (Albion Online)**: 목표 설정으로 디자인 제한, 관계 구축 후 수학적 모델링으로 밸런스를 사전 검증하세요.
- **플레이테스트 우선 (Ian Schreiber)**: 플레이테스트, 분석, 수학적 모델링을 균형 있게 사용하세요.
- **반복적 개선**: 설계 → 테스트 → 조정의 사이클을 유지하고, 데이터 기반으로 지속적으로 개선하세요.
- **통계적 검증**: 충분한 샘플 크기와 반복 횟수로 통계적 유의성을 확보하세요.

### 수리적 검증 도구
- **LP 메타게임 예측 (Alex Jaffe)**: 매치업 행렬을 제로섬 게임의 보수 행렬로 놓고 선형계획법으로 최적 혼합 전략을 계산하면(몇 줄 코드로 충분) 출시 전에 지배 캐릭터를 예측할 수 있습니다. 주의: 흔한 분석인 '행 합계(평균 상대 승률)'는 무의미 — 실전 상대는 평균이 아니라 '강자를 이겨서 살아남은 강자'입니다.
- **이중 풀이 검증**: 같은 확률 문제를 수식과 몬테카를로 양쪽으로 풀어 일치하면 검증 완료. 확률 작업엔 내장 디버거가 있습니다: 0~1 범위, 비중첩 결과의 합 = 정확히 1. 반복 시스템은 마르코프 체인으로 정확해를 구하세요.
- **밸런스 문제의 99%는 스프레드시트로**: AI 구현 1,000줄짜리 문제도 50개 셀로 재구성됩니다. "분석하기 너무 어려운 시스템은 게임에 넣기에도 너무 복잡하다" — 분석 가능성이 스코프 억제 장치를 겸합니다. 시트 → 게임 데이터 내보내기는 반드시 자동화하세요(수작업 오타가 "게임 깊숙한 곳 10배" 사고를 냅니다).
- **랜덤 AI 도달률 시각화**: 무작위 행동 AI로 수천 회 자동 플레이스루를 돌려 노드별 도달률을 시각화하세요 — 도달률 0% 노드는 구현 버그 신호입니다. 코드보다 데이터 분포가 문제인 경우가 많습니다.

### 테스트 운영
- **베타는 밸런스 테스트에 부적합 (Riot)**: 유저는 어차피 사라질 변경에 진심으로 임하지 않고 인구가 적어 매칭도 왜곡됩니다. 전직 프로로 구성된 사내 테스트 팀을 운영하세요 — 2주 패치 주기의 실작업 시간은 3일뿐이라 필수입니다.
- **테스터 숙련도 편향을 보정하라 (사쿠라이)**: 개발팀 기준으로 맞춘 난이도는 일반 유저에게 통하지 않습니다(Case 8). 테스트 결과를 읽을 때 테스터 표본의 숙련도 분포를 함께 기록하고, 타깃 유저 표본이 없다면 그 데이터로 난이도를 확정하지 마세요.
- **KPI와 리스크 완화 계획을 출시 전에 (Riot)**: 최고의 설계도 명중률 70%입니다. "이 챔피언이 라인 불리가 되면 Q를 너프한다"처럼 실패 시나리오와 조치를 사전 합의하면 "목표 미달인가"를 논쟁하지 않고 즉시 실행합니다.
- **난이도 천장은 의도적 과튜닝으로 실측 (God of War)**: '너무 쉬움' 기준을 확보한 상태에서 한 회차를 일부러 과하게 어렵게 튜닝해 하드 실링을 확인하고 되돌립니다 — 밸런스의 양 끝을 실측해 스위트 스폿을 좁히는 방식입니다.
- **출시 = 최대 규모 QA (Albion Online)**: 동시 접속 10,000명 = 첫 1시간에 10,000맨아워의 QA. 사전 테스트로는 절대 못 따라가므로 가격 채널 같은 자동 안전장치를 심고 출시 후 반복 수정하세요. "당신이 예측 가능한 게임은 플레이어도 풀 수 있다."

### 수학의 한계
- **"스프레드시트가 아름다워도 게임을 플레이하라" (Extra Credits)**: 수치상 완벽히 균형 잡힌 시스템이 체감상 완전히 불균형할 수 있습니다(브레이크 포인트). 체감 문제가 크면 수식을 깨서라도 느낌을 맞추세요 — 플레이어가 경험하는 것은 후드 아래의 수학이 아니라 느낌입니다.

## 관련 문서 (See Also)

- [종합 밸런싱 설계 가이드](150-balancing_overview.md)
- [스탯·드롭·확률 테이블](152-stats_drop_probability_tables.md)
- [QA 계획 & 테스트 케이스](231-qa_plan_test_cases.md)
- [매치3 퍼즐 설계 분석](448-Match3-Review.md)
