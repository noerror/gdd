...
### 5.4 콘텐츠 스키마 패턴
- **Verb Vocabulary (동사 어휘)**: 퀘스트·행동 스텝을 '주동사+대상'(예: obtain/kill + 대상 ID)의 유한 목록으로 정의하는 방식. 직군 간 공용 언어와 자동 생성의 기반
- **Flag Gating (플래그 게이팅)**: 이벤트 발동을 데이터 파일의 요구조건(플래그) 목록으로 제어하는 패턴. 조건 어휘는 필요 사례가 생길 때마다 증설
- **파라미터 표기 규약**: 사양서의 파라미터명을 【】로 묶고 조정 수치는 ○로 표기하여, 사양서와 파라미터 파일이 동일 문자열로 1:1 대응되게 하는 규칙 (사쿠라이 마사히로)
- **Blackboard**: 행동트리 등 로직이 참조하는 중앙 속성 DB. Decorator(abort·쿨다운·루프·태그·브레이크포인트)와 서브트리 별도 리소스화가 유지보수의 3요소
...
- [ ] 속성의 타입과 제약이 명시되었는가?
- [ ] 스키마가 확장 가능한가?
- [ ] 퀘스트·행동 등 콘텐츠 스텝의 동사·조건 어휘가 유한 목록으로 관리되는가?
- [ ] 파라미터명이 사양서·데이터 파일·툴에서 동일 문자열로 통일되는가?
- [ ] 행동트리 등 로직 스키마에 디버깅 필드(태그·브레이크포인트·쿨다운)가 포함되는가?
...
**Case 8: 퀘스트 동사 어휘 (Horizon Zero Dawn)**
- **좋은 예 (Good)**: "퀘스트 스텝을 obtain/kill 등 유한한 '주동사+대상' 어휘로 정의, 작가-디자이너-엔지니어의 공용 언어가 되고 퀘스트 자동 생성 기능까지 추가 비용 없이 파생"
- **안티패턴 (Bad)**: "동사 목록에 중복·1회용 특수 동사가 무분별하게 추가되어 어휘가 사실상 자유 텍스트로 퇴화, 자동화·검증 불가"

**Case 9: 대화 조건 게이팅 (Hades)**
- **좋은 예 (Good)**: "NPC 대화 데이터에 '1회 이상 대화했는가'부터 '이번 런에 물고기를 잡았는가'까지 요구조건 플래그를 사례가 생길 때마다 증설, 수만 라인 대사의 발동 조건을 전부 데이터로 관리"
- **안티패턴 (Bad)**: "모든 조건을 사전에 예측해 스키마를 확정하려다 착수가 지연되고, 미지원 조건은 코드 하드코딩 분기로 회귀"

**Case 10: 파라미터 표기 규약 (대난투 스매시브라더스)**
- **좋은 예 (Good)**: "사양서에 '【기술 개시 시 X속도】는 ○으로 이동 개시'처럼 파라미터명을 괄호로 고정하고 수치는 ○로 표기, 파라미터 파일에 같은 문자열이 그대로 놓여 비담당자도 조정 지점을 즉시 식별"
- **안티패턴 (Bad)**: "사양서 서술과 데이터 필드명이 달라, 수치를 바꿀 때마다 담당 프로그래머에게 어느 필드인지 구두 확인 필요"
...
- **유한 동사 어휘가 자동화를 낳는다 (Horizon Zero Dawn)**: 콘텐츠 스텝을 유한 동사 목록으로 강제하면 검증·자동 생성·직군 간 소통이 전부 그 위에서 파생됩니다. 어휘 목록의 중복·과잉 특수 항목을 정기 정리하는 관리 주체를 지정하세요.
- **조건 어휘는 완전 예측하지 말고 증설하라 (Hades)**: 발동 조건 종류를 처음부터 전부 예상하는 것은 불가능합니다. 조건 추가가 스키마 변경 없이 가능한 확장 구조(키-값 요구조건 목록)로 설계하고, 신규 조건 추가 절차를 문서화하세요.
- **디버깅 장치는 스키마의 1급 시민 (Remedy)**: 행동트리·상태머신 스키마에 태그, 브레이크포인트, abort/쿨다운 Decorator를 처음부터 포함하고, 서브트리는 별도 리소스로 분리해 재사용·단독 테스트가 가능하게 설계하세요.
- **사양서와 데이터의 문자열 통일 (사쿠라이 마사히로)**: 【】로 묶인 이름은 반드시 파라미터화한다는 규칙 하나로 사양서-데이터-툴이 자동 연결됩니다. 파라미터는 여러 명이 함께 조정하므로 이름만으로 의미가 통해야 합니다.
- **전략·난이도 테이블도 외부화 대상 (Halo Wars 2)**: AI 커맨더의 매크로 전략 5종·마이크로 행동 10종을 데이터 테이블로 관리해 출시 후 코드 수정 없이 패치로 조정. 밸런스 수치뿐 아니라 행동 선택 로직도 스키마화할 수 있습니다.
- **범용 블록 + 예외 스크립트의 이중 구조 (GoldenEye 007)**: 디자이너가 사전 컴파일된 원자 행동 블록을 문맥별 시퀀스로 조립하는 범용 시스템을 기본으로 하되, 주요 인물은 레벨별 비스포크 스크립트를 허용하는 이중 구조가 현실적입니다. 예외의 존재 자체를 스키마 설계에 반영하세요.
