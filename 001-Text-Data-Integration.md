...
**텍스트 기반 기획/데이터 연동 환경**
1. **키 컨셉** : 마크다운 SSoT, 버전관리, LLM/RAG, 코드블록 임베딩 등 핵심 방향.
2. **마크다운 기반 문서 시스템** : Obsidian·Typora·VSCode/LLM 도구와 협업 장점, 확장 기능.
3. **데이터 임베딩 시스템** : 코드블록 임베딩 규칙, 포맷, 추출 스크립트, 워크플로우.
4. **활용 아이디어 예시** : 다이얼로그/번역/QA·밸런스 로그, LLM 생성·자동 참조 활용 사례.
...
- 코드 블록 데이터 임베딩 / 파싱 자동화 / 기획↔구현 일치

# 2. 마크다운 기반 문서 시스템

## 2-1. 개발 환경
1. **Obsidian**: 충실한 마크다운 기반 에디팅 환경 (Notion 의 로컬 버전 느낌)
  - 직관적인 아웃라이너, 경로 무시 파일 링크 지원, 역참조 확인 등 다양한 편의 기능
  - 그래프, 캘린더, 캔버스, DB 검색, 칸반(Trello 등) 다양한 플러그인 지원
  - 클립보드에서 이미지 삽입 가능(파일 업로드와 연동), YouTube 영상 및 외부 이미지 임베딩 표시
2. **Typora**: 실시간 미리보기와 경량 편집 환경
3. **VSCode,Cursor**: 개발 환경과 LLM 연동을 묶어 코드, 기획 동시 편집을 지원

## 2-2. 장점
1. **Git,SVN**: 텍스트 기반이라 머지가 용이하고, 브랜치, 이력 관리도 용이(history 변경 확인, blame 으로 라인별 작성자와 작성일 확인)
2. **LLM 연동 자동화**: RAG 기반 문서 임베딩으로 기획 컨텍스트를 호출하고, 코드 생성, 일관성 검증, 자동 문서화를 한 번에 처리
3. **협업 동기화**: 프로그래머가 프로그래밍 개발 환경에서 동일한 마크다운 소스를 즉시 열람하여 실시간으로 기획 변경 사항을 공유합니다.
...
**코드 블록 임베딩:**
- 마크다운 코드 블록 내 데이터 작성 / 표준 문법 : 언어명 + 콜론 + 파일명/ID
...
- **자동 문서화**: 코드 블록 데이터 읽고 설명 문서 자동 생성
...
- **웹 기반 도구 제작 (Vibe Coding)**: FastAPI 등을 활용해 기획 데이터의 시각화 및 수정이 가능한 웹 툴을 신속하게 개발합니다.
  ...
  - **엑셀 <-> 마크다운 변환기**: 기존 엑셀 기획서를 마크다운 코드 블록으로 자동 변환 / 역변환으로 비개발자 협업 지원
