---
title: 퍼즐 & 규칙 조합 모듈 (Puzzle & Gimmick) 작성 가이드
description: 퍼즐과 규칙 조합 메커닉을 플레이어 문제 해결을 지원하도록 설계하여 창의적 사고와 전략적 사고를 유도하는 모듈 디자인 가이드
---
...
- **제외**: 순발력을 요하는 액션 컨트롤([실시간 액션 모듈](302_Real-time_Action_Module_Guideline.md) 참조), 스토리 대화 선택지([내러티브 & 퀘스트 모듈](308_Narrative_Quest_Module_Guideline.md) 참조), 전투 밸런스
...
- **캐치 (Catch)**: "A를 하면 B가 불가능해지는" 퍼즐의 핵심 모순. 좋은 퍼즐은 캐치(모순)→가정(당연해 보이는 풀이로 유도한 첫 실패)→계시(규칙의 비자명한 귀결 발견)의 3단으로 작동
...
- [ ] 각 퍼즐에 캐치(모순)·가정(유도된 첫 실패)·계시(발견)의 3요소가 있는가?
- [ ] 정답을 깨달은 후 실행이 수 초 안에 끝나는가? (실행이 어려우면 퍼즐이 아니라 손재주 시험)
- [ ] 퍼즐 배열 기준(신규 개념 수, 해법 수, 필요 수순 수)이 정량화되어 있는가?
- [ ] 힌트 노출까지의 지연 시간과 힌트 끄기 옵션이 정의되어 있는가?
...
**Case 4: 답의 노출 거리**
- **좋은 예 (Good)**: "같은 구성 요소를 분리 배치해 연결의 발견을 플레이어 몫으로 남김" (Portal 2)
- **안티패턴 (Bad)**: "라이트브리지가 이미 버튼 위에 놓인 채 시작" (The Turing Test - 사실상 동일한 퍼즐이 배치 때문에 시시해진 비교 사례)

**Case 5: 힌트 개입 시점**
- **좋은 예 (Good)**: "힌트 빈도와 지연 시간을 옵션으로 조절 가능" (A Plague Tale)
- **안티패턴 (Bad)**: "퍼즐 도착 수십 초 만에 동료 NPC가 답을 말해버림" (God of War Ragnarök - 완주율 강박이 낳은 성취감 박탈)

**Case 6: 추리 검증 방식**
- **좋은 예 (Good)**: "3인의 신원 세트가 전부 맞아야 확정되어 찍기가 무력화됨" (Return of the Obra Dinn)
- **안티패턴 (Bad)**: "정답 후보를 객관식으로 나열" (추리 대신 소거법·찍기로 전락)
...
- **퍼즐 제작은 발명이 아니라 큐레이션 공정이다 (Jonathan Blow)**: 이 모듈이 명세할 것은 개별 퍼즐이 아니라 '메카닉에서 현상을 전수 탐색해 걸러내는 공정'과 폐기 기준입니다. 목표는 "겨우 풀었다"가 아니라 "이해했다"입니다. (공정 단계 상세: [114](114-puzzle_special_systems.md))
- **규칙은 예외 없이 전면 적용하라 (Braid)**: 속성 하나를 정했으면 열쇠·문·플랫폼 등 모든 오브젝트에 일관 적용해 파급을 끝까지 탐색하세요. 예외가 늘수록 플레이어의 추론이 무너집니다.
- **'지우면 왜 재미있는가'부터 물어라 (사쿠라이 마사히로)**: 테트리스의 게임성은 쌓임(리스크)과 모아서 한꺼번에 지우기(리턴)의 구조에서 나옵니다. 장르 관습의 동사를 원리 수준에서 이해하면 갈아끼울 수도 있습니다 (메테오스의 '지우기'→'쏘아 올리기').
- **막힘 대비는 동시 목표로**: 어드벤처형 퍼즐은 동시에 추적 가능한 목표를 여러 개 열어 두어, 한 퍼즐에 막혀도 다른 곳에서 진전을 얻게 하세요 (원숭이 섬 계열의 표준 구조).
- **요소 수는 최소로**: 퍼즐의 요소가 많을수록 대부분은 소음이 되고 리셋만 고통스러워집니다. 통찰 전달에 필요한 최소 셋업이 좋은 퍼즐의 형태입니다.
- **오답에 제네릭 응답 금지**: "그건 안 돼" 같은 무의미한 거절 대신, 왜 안 되는지와 다음 사고 재료를 함께 주세요. 실패는 처벌이 아니라 정보 제공이라는 설계 목표의 실행 규칙입니다.
