...
### 9.2 레퍼런스 발굴 전략
- **가장 증류된 교본을 병기하라 (GMTK 분석)**: 낯선 장르의 원리는 최고작보다 가장 단순한 대표작이 더 잘 보여줍니다 — RTS의 자원-방어 딜레마는 Starcraft보다 Plants vs. Zombies(해바라기=경제 vs 공격 화력)가 명확합니다. 레퍼런스 목록에 '최고작'과 '교본작'을 구분해 병기하세요.
- **레트로 볼트 채굴**: Cuphead(고전 액션 보스), Super Blood Hockey(NES Ice Hockey), Gravity Ace(Thrust/Gravitar)처럼 잊힌 고전의 현대화가 검증된 발굴 전략입니다. 고전을 플레이하며 "무엇이 이 게임을 위대하게 했나"를 기록하세요.
- **게임 밖에서 캐라**: 포켓몬은 타지리 사토시의 곤충채집에서, 피크민은 미야모토 시게루가 관찰한 정원의 개미 행렬에서 태어났습니다. 새 취미, 안 보던 장르의 영화·책이 레퍼런스 풀을 넓힙니다.
- **나쁜 게임을 플레이하라 (Scott Rogers)**: 잘 만든 게임보다 못 만든 게임이 '무엇이 왜 나쁜가'를 드러내 개선 아이디어를 더 많이 자극합니다. 무드보드에도 '반면교사' 슬롯을 두세요.
